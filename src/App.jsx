...
function ScheduleApp({ user, profile, onLogout }) {
  ...
  const cellShifts = (agentId, dayIndex) => shifts.filter((s) => s.agent_id === agentId && s.day_index === dayIndex)

  const addShift = async (agentId, dayIndex, shiftData) => {
      ...
      <div style={{ flex: 1, minWidth: 0, background: '#fafaf7' }}>
        ...
        <div style={{ padding: 24 }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: 40, color: '#9b9a8f' }}>Aucun agent pour cette agence.</div>
          ) : (
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', background: '#fff', borderRadius: 10, overflow: 'hidden', border: '1px solid #ebe9e2', fontSize: 13 }}>
                <thead>
                  <tr>
                    <th style={{ border: '1px solid #ebe9e2', padding: 10, background: '#fafaf7', fontSize: 12, color: '#9b9a8f', textAlign: 'left', minWidth: 150, fontWeight: 500 }}>Agent</th>
                    {DAYS_SHORT.map((d, i) => (
                    ...
                    <tr key={agent.id}>
                      <td style={{ border: '1px solid #ebe9e2', padding: '8px 12px', whiteSpace: 'nowrap' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          ...
                          {canEdit && <span onClick={() => removeAgent(agent.id, agent.name)} style={{ cursor: 'pointer', color: '#c15c5c', fontSize: 11 }}>✕</span>}
                        </div>
                      </td>
                      {DAYS_SHORT.map((_, d) => {
                        const cell = cellShifts(agent.id, d)
                        const pickerKey = `${agent.id}-${d}`
                        return (
                            ...
                            {cell.map((s) => {
                              ...
                              return (
                                <div key={s.id} style={{
                                  ...
                                  <span>{label}</span>
                                  {canEdit && <span onClick={() => removeShift(s.id)} style={{ cursor: 'pointer', opacity: 0.5, fontSize: 11 }}>✕</span>}
                                </div>
                              )
                            })}
                              ...
                              <div>
                                ...
                                {pickerOpen === pickerKey && (
                                  <div style={{ position: 'absolute', zIndex: 10, top: '100%', left: 0 }}>
                                    <ShiftPicker
                                      onSelect={(data) => addShift(agent.id, d, data)}
